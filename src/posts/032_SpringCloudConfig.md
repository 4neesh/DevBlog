...
&#8226; Use Spring Cloud Config to create an externalised configuration in a distributed system.<br>
...
<p>
The Config server can use many different medium for storing the configuration files including JDBC, Redis, or AWS S3. The default storage is Git and will be used in this demo.
</p>
<p>
Once subscribed, services within the system can use the Config server as a REST service to obtain their configuration values.
In this example, I will use GitHub to host the configuration server.
...
<p>
The Spring Config server uses the Spring Config Server dependency and Spring Web dependency together. The Spring Web dependency provides the REST functionality of the server once all the 
...
<p>
The config server must be configured to know where to read the .properties files from, which is done in the config server application.properties file:
...
The Config server will register all .properties files within the folders defined by the comma-separated list.
</p>
...
I have manually created these files and placed them within the folders at the Git repository to allow the server to build its key-value pairs of properties:
...
<p>
When the config server has started, you will be able to send GET requests to the 8888 port to obtain the relevant value(s). In the next section, we will look at the exact endpoints that are generated by the config server and how the values returned are determined.
</p>

<br>
<h4>Navigating the Config server</h4>
<p>
When the Config server is launched, it will use the Spring-web starter to expose endpoints that reflect the properties files which it encounters. 
A GET request can be made to these endpoints using the following name convention:
...
Towards the bottom are the config properties returned from the Config server under 'propertySources'. The first set of values come from the food-prod properties file and the second set of values are from the application-prod properties file. The config server has returned the properties files that are appropriate to the application name and the profile of prod first, followed by the default application-prod properties second. 
</p>
<p>
...
![to do ](../images/032_springLoad.png)
</p>
...
The spring.application.name value will determine the application name for which to obtain the properties file for. This property is defaulted to 'application'.
The spring.profiles.active property will tell the application which profile it will assert for the config property search. This property is defaulted to 'default'. 
...
Environment=PROD
```
The Environment property has been taken from the application-prod.properties file which is read second to the drink-prod.properties file. As the drink-prod file did not contain a value for the 
...
server.port=7777
```
The new property for server.port will instruct the client to be hosted on the 7777 port. The property would only ever work for a single client as only one client can be registered to port 7777.
